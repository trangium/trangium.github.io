...
class MultiTargetSolver {
    ...
    struct ProductDistanceTable {
        int c0 = -1, c1 = -1;   // indices into groups_
        int size1 = 0;           // state count of c1 (= stride of c0; stride of c1 is 1)
        long long total_states = 0;
        Mod3Table distance_table;
        long long identity_id = -1;
    };
    ...
    uint64_t hashCompact(const std::vector<PiecePerm>& cp, const IncompleteGroup& incg) const {
        ...
        for (int t = 0; t < (int)spec.types.size(); t++) {
            ...
            for (int p = 0; p < ty.count; p++) {
                int cls_p = incg.piece_to_class[t][p];  // class of piece p (source)
                if (cls_p >= 0) {
                    int val   = cp[t][p] - ty.base;
                    int dest  = val / ty.m;              // current position of piece p
                    int twist = val % ty.m;
                    int reduced = twist % spec.classes[cls_p].orientation_mod;
                    h ^= incg.zobrist_op[type_offset + dest][cls_p * incg.max_orient_mod + reduced];
                }
            }
            type_offset += ty.count;
        }
        return h;
    }
    ...
    void buildProductDistanceTables() {
        ...
        for (auto& pdt : product_tables_) {
            if (pdt.c0 < 0 || pdt.c1 < 0) continue;

            const int size0 = (groups_[pdt.c0].kind == TargetGroup::ORIENTPERM)
                              ? (int)groups_[pdt.c0].op_spec.total_states
                              : (int)groups_[pdt.c0].canon_id_table.size();
            pdt.size1 = (groups_[pdt.c1].kind == TargetGroup::ORIENTPERM)
                        ? (int)groups_[pdt.c1].op_spec.total_states
                        : (int)groups_[pdt.c1].canon_id_table.size();
            pdt.total_states = (long long)size0 * pdt.size1;
            pdt.identity_id = (long long)groups_[pdt.c0].identity_id * pdt.size1
                            + groups_[pdt.c1].identity_id;
            ...
            while (any_new) {
                ...
                for (long long s = 0; s < N; s++) {
                    if (pdt.distance_table.get(s) == depth_mod3) {
                        for (int mi = 0; mi < nMoves; mi++) {
                            int new_c0 = groups_[pdt.c0].transition_table[comp0 * nMoves + mi];
                            int new_c1 = groups_[pdt.c1].transition_table[comp1 * nMoves + mi];
                            long long ns = (long long)new_c0 * pdt.size1 + new_c1;
                    ...
                    if (++comp1 >= pdt.size1) { comp1 = 0; comp0++; }
                }
                depth_mod3 = (depth_mod3 + 1) % 3;
            }
        }
    ...
    // Greedy walk on a product distance table.
    int computeExactH_product(int pd_idx, long long s) const {
        ...
        while (s != pdt.identity_id) {
            ...
            int comp0 = (int)(s / pdt.size1);
            int comp1 = (int)(s % pdt.size1);
            for (int mi = 0; mi < nMoves; mi++) {
                int new_c0 = groups_[pdt.c0].transition_table[comp0 * nMoves + mi];
                int new_c1 = groups_[pdt.c1].transition_table[comp1 * nMoves + mi];
                long long ns = (long long)new_c0 * pdt.size1 + new_c1;
    ...
    int idaDfs(std::vector<std::vector<int>>& ss, std::vector<std::vector<int>>& hs,
        ...
        for (int mi = 0; mi < nMoves; mi++) {
            ...
            for (int p = 0; p < np; p++) {
                ...
                long long ns = (long long)ss[g+1][pdt.c0] * pdt.size1 + ss[g+1][pdt.c1];
...
public:
    ...
    void addProductDistanceTable(int c0, int c1) {
    ...
    std::vector<int> solve(const std::vector<int>& startPerm,
        ...
        for (int pi = 0; pi < np; pi++) {
            ...
            long long s0 = (long long)state[pdt.c0] * pdt.size1 + state[pdt.c1];
...
EMSCRIPTEN_BINDINGS(module) {
    ...
    emscripten::class_<MultiTargetSolver>("MultiTargetSolver")
        ...
        .function("addProductDistanceTable",    &MultiTargetSolver::addProductDistanceTable)
