import { identity, type Perm, CycPerm } from "./Perm.js";
...
export function canonStr(sgs: CycPerm[][], state: Perm): string {
  return canonicalize(sgs, state).toString();
}

export function schreierSims(g: Perm[]): CycPerm[][] {
  const n = g[0].n;
  const e = identity(n);
  let sgs: Perm[][] = [];
  let sgsi: Perm[][] = [];
